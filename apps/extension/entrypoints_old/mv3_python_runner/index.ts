...
const sendResponse = async (payload: PythonOffscreenResponsePayload): Promise<void> => {
...
const initPyodide = async (): Promise<PythonRunnerContext> => {
    ...
    for (const pkg of SUPPORTED_PYTHON_PACKAGES_PYPI) {
        ...
        try {
            ...
            throw new Error(`Failed to install Python package ${pkg}: ${getErrorMessage(error)}`);
        }
    }
...
const handleOffscreenRequest = async (payload: PythonOffscreenRequestPayload): Promise<void> => {
...
browser.runtime.onMessage.addListener((message: unknown) => {
    ...
    void handleOffscreenRequest(message.payload).catch(async (error: unknown): Promise<void> => {
        await sendResponse({
