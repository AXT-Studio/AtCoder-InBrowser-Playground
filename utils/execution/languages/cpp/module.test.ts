...
`,
    ...
    it("main が無いと CE になる", async () => {
        const outcome = await runPrepared(ctx, `int foo() { return 0; }`, "");
        expect(outcome.status).toBe("CE");
        expect(outcome.stderr).toMatch(/int main/);
    }, 30_000);
...
int main() {
    atcoder::dsu uf(3);
    uf.merge(0, 1);
    std::cout << uf.same(0, 1) << " " << uf.same(0, 2) << std::endl;
    return 0;
}
`,
            "",
        ...
        expect(outcome.status).toBe("completed");
        expect(outcome.stdout).toContain("1 0");
    }, 120_000);
});
