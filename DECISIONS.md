...
- `wasm-unsafe-eval` 必須
- Pyodide / wheel / ruby.wasm / WASI Clang は拡張内同梱。`connect-src 'self'`（CDN への実行時依存はしない）
...
- `cpp` は WASI Clang（Clang 21.1.0）。`prepare` でコンパイルし、`run` は WASI 実行だけ。UI 名は **C++(Clang)**
...
- 成果物は `pnpm run build:engine:qjs-wamr` で生成し、リポジトリには置かない。`dev:` のたびに自動ビルドはしない
...
- 対象ジャッジは **C++23 (Clang 21.1.0)**。実行も **同じ 21.1.0** の自前 wasm（libc++、wasi-sdk 28、target `wasm32-wasip1`）。Function()/eval は使わない
...
- ツールチェインは `pnpm run build:engine:clang`（`engine/clang-wasi/scripts/build.sh`）で wasm 化し、`plugins/cppPublicAssetsHook.ts` が `engine/clang-wasi/dist` + shim + ACL を `assets/cpp/` へ同梱。glue JS は Blob URL 経由で `import()` しない（Firefox CSP が `blob:` を弾く）。拡張内 URL を直接 import する
- Chrome / Firefox とも他言語と同じ実行ホスト（Offscreen / Background → Runner Worker）。`'unsafe-eval'` は付けない
