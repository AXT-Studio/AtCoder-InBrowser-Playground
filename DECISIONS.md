...
| TLE            | ready 後のみ計測。Host がタイマー＆ terminate。結果型は `CodeTestResult`（§4.5–4.6） |
...
- 初期化コストを制限時間に含めない。**Exec Host が `ready` を受けてから** `timeLimitMs` のタイマーを開始する
- Runner Worker は制限時間を知らない（時計と `terminate` は Host の責任）
- **Verdict の TLE** は壁時計全体ではなく、**ready 以降のユーザーコード実行時間のみ**（ジャッジ忠実）
- TLE 時は Host が Worker を `terminate` し、結果を Host 側で合成する（Worker からの `result` は来ない／遅延分は無視）
...
### 4.5 実行結果型 `CodeTestResult`（決定）

Runner / Exec Host が返す実行結果は次に統一する。**AC/WA は含めない**（Runner は判定できない。Content が mode ごとに判定する）。
...
type CodeTestResult = {
    ...
    stdout: string;
    stderr: string; // RE / CE のエラー理由もここ。TLE 時は "" または "Time Limit Exceeded." 程度で可
};
```

| `status`    | 意味                                                |
| ----------- | --------------------------------------------------- |
| `completed` | ユーザーコード実行が制限内に終わった（AC ではない） |
| `CE`        | **ready より前**の失敗（準備・変換・init 等）       |
| `RE`        | **ready 以降**の失敗（実行時エラー等）              |
| `TLE`       | ready 後に Host が打ち切り                          |

- `execTime`: 基本は **ready〜終了**。`TLE` は打ち切り時点の経過。`CE` は **`-1`**
- TLE で Worker を terminate するため、途中の stdout/stderr は原則得られない → 空文字（stderr に TLE 文言を入れるのは可）。判定の正本は `status === "TLE"`

### 4.6 層間プロトコル（決定）

メッセージ／呼び出しのフィールド名は **`language` に統一**（`lang` は使わない）。

```
Content
  → execRequest { id, language, code, stdin, timeLimitMs }
Background（ハブ）
  → Exec Host へ渡す
       Chrome MV3: Offscreen へ転送（メッセージ）
       Firefox MV2: 同一 Background 内の関数呼び出し（自己 sendMessage はしない）
Exec Host
  → 言語に応じた Runner Worker を起動
  → この時点で result の受付を開始する（ready より result が先に届くレースに備える）
  → start { id, language, code, stdin }  ※ timeLimitMs は載せない
Runner Worker
  → 準備完了で ready { id }
  →（続けて）ユーザーコード実行
  → 完了で result { id, result }
Exec Host
  → ready でタイマー開始
  → result が先 → タイマー停止し、その codeTestResult を採用
  → タイマーが先 → Worker を terminate し、TLE の CodeTestResult を合成
  → 終わったら Worker を terminate（まだ生きていれば）
  → execResponse { id, codeTestResult }
Background
  → Content へ execResponse { id, codeTestResult }
Content
  → mode に応じて AC/WA 等を判定・表示
```

**Host 側の注意:**

1. Worker 起動直後から `ready` / `result` の両方を受け付ける（リスナー後付けで取りこぼさない）
2. 同一 Worker への `postMessage` は順序が保たれる前提。Worker は必ず `ready` のあと `result` を送る
3. すでに TLE／完了した `id` への遅延 `result` は無視
4. `execTime`（completed / RE）は Host が **ready 受信時刻〜 result 受信時刻** で埋めてよい（Worker 計測に頼らなくてもよい）。CE は `-1`、TLE は打ち切り時点の経過

**並列:** リクエストは `id` で識別する。並列実行は残してよい。
...
- 生成器テンプレは別物として用意
- Python テンプレは未決（わからん）
- **TODO（移植時）:** 先頭コメントのバッファ種別（旧は固定で `Submission`）を引数化し、Solve / Naive / Generator で差し替える（例: `generateTemplate(..., role: "submission" | "naive" | "generator")`）
...
- Python 提出用テンプレの中身
- 通常テンプレをどこまで削るか
- テンプレ先頭コメントのバッファ種別引数化（§7.3 TODO。テンプレ移植と同時に）
